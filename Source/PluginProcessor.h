...
private:
    using Filter = juce::dsp::IIR::Filter<float>; // Each filter response is 12 dB/oct per filter type
    ...
    void updateCutFilter(ChainType& leftLowCut, const CoefficientType& cutCoefficients, const Slope& lowCutSlope)
    {
        ...
        /*
        for (int i = 0; i <= lowCutSlope; i++)
        {
            *leftLowCut.template get<i>().coefficients = *cutCoefficients[i];
            leftLowCut.template setBypassed<i>(false);
        } */
